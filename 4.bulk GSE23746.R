...
eset <- getGEO(filename = 'Data/Rawdata/GSE23746_series_matrix.txt.gz',getGPL = F)
...
table(group$Group)
group$Group <- ifelse(group$Group=='Male Subject with Carotid Disease','CS','Con')
...
save(GSE23746_cc,GSE23746_ee,file = 'CEA_data/GSE23746.rda')


# -------------------------------------------------------------------------
rm(list = ls())
load('CEA_data/GSE23746.rda')
