...
eset <- getGEO(filename = 'GSE163154_series_matrix.txt.gz',getGPL = F)
...
table(group$Group)
group$Group <- ifelse(group$Group=='non-IPH','Con','IPH')
...
save(GSE163154_cc,GSE163154_ee,file = 'CEA_data/GSE163154.rda')


# -------------------------------------------------------------------------
rm(list = ls())
load('CEA_data/GSE163154.rda')
