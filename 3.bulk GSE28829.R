...
eset <- getGEO(filename = 'Data/Rawdata/GSE28829_series_matrix.txt.gz',getGPL = F)
...
table(group$Group)
group$Group <- ifelse(group$Group=='advanced atherosclerotic plaque','CS','Con')
...
save(GSE28829_cc,GSE28829_ee,file = 'CEA_data/GSE28829.rda')



# -------------------------------------------------------------------------
rm(list = ls())
load('CEA_data/GSE28829.rda')
