rm(list = ls())
library(stringr)
library(dplyr)
library(limma)
library(data.table)
library(tibble)
library(BioEnricher)
library(stringr)
library(data.table)
library(tibble)
library(dplyr)
library(GEOquery)

eset <- getGEO(filename = 'Raw data/GSE100927_series_matrix.txt.gz',getGPL = F)
expr <- as.data.frame(exprs(eset))
range(expr)
clin <- pData(eset)
clin <- clin[,c(2,1)]
group <- clin
rm(eset,clin)

group <- group %>%
  filter(str_detect(title, "Carotid artery_")) %>%  # 挑出含颈动脉的行
  mutate(title = str_extract(title, "Carotid artery_[^ ]*"))  # 只保留颈动脉字段

colnames(group) <- c('Sample','Group')
rownames(group) <- NULL
group <- tibble::column_to_rownames(.data = group,var = 'Sample')
table(group$Group)
group$Group <- ifelse(group$Group=='Carotid artery_Control','Con','CS')

#数据转换
load('GPL17077.rda')
# ann <- ann[,c(2,3)]
# nchar('ENSG00000223972')
# ann$ENSG <- str_sub(ann$ENSG,1,15)
exp <- merge(ann,expr,by.x = 1,by.y = 0)[,-1]
#保留平均表达量最大的基因
exp1 <- exp %>%
  group_by(SYM) %>%
  summarise(across(where(is.numeric), mean)) %>%  # 你也可以换成 max()
  ungroup()

rownames(exp1) <- NULL
exp1 <- tibble::column_to_rownames(exp1,'SYM')
group1 <- tibble::rownames_to_column(group,'Sample')
exp2 <- exp1[,colnames(exp1)%in%group1$Sample]
exp3 <- apply(exp2,2,as.numeric)
exp3 <- as.data.frame(scale(exp3))
rownames(exp3) <- rownames(exp2)
range(exp3)

GSE100927_cc <- group1
GSE100927_ee <- exp3

save(GSE100927_cc,GSE100927_ee,file = 'CEA_data/GSE100927.rda')


# -------------------------------------------------------------------------
rm(list = ls())
load('CEA_data/GSE100927.rda')








